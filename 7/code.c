...
#define TRUE 1
#define FALSE 0 #define ML 1024
#define MPROC 32

/* 
Function to create a new connection to port 'connect_to' 1. Creates the socket. 
2.	Binds to port. 
3.	Returns socket id 
*/
...
int main(int argc, char *argv[])
{
    ...
    sock_id = connect_to_port(self); // 3. if not the initiator wait for someone else
    while (TRUE)
    {
        printf("\t - - - - - - - - - - - - - - - - - - - - - - - - - -\n\n");
        sleep(2);
